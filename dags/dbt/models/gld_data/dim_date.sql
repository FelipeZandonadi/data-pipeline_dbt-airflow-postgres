...
WITH
    dates AS (
        SELECT
            {{ catch_data_information('sto.order_approved_at') }}
        FROM slv_data.slv_tb_orders AS sto
        WHERE sto.order_approved_at IS NOT NULL
        UNION
        SELECT
            {{ catch_data_information('sto.order_delivered_customer_date') }}
        FROM slv_data.slv_tb_orders AS sto
        WHERE sto.order_delivered_customer_date IS NOT NULL
        UNION
        SELECT
            {{ catch_data_information('sto.order_estimated_delivery_date') }}
        FROM slv_data.slv_tb_orders AS sto
